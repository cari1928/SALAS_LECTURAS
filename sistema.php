...
//----------------------------------------------------------------------------------------
    ...
    //        {
	//			$this->error('Usted no tiene el rol correcto');
	//		}
	//	}
	//	else
	//	{
	//		$this->error("Usted no ha iniciado sesion");
	//	}
	//}

	/*public function checkPrivilegios ($privilegio)
	{
		if($_SESSION['logueado'])
		{
            if(is_array($privilegio))
            {
                $anyPrivilege = false;
                foreach($privilegio as $p)
                {
                    if(in_array($p, $_SESSION['privilegios']) && !$anyPrivilege)
                        $anyPrivilege = true;
                }
                if(!$anyPrivilege)
                    $this->error('Usted no cuenta con los privilegios necesarios.');
            }
            else if(!in_array($privilegio, $_SESSION['privilegios']))
                $this->error('Usted no cuenta con los privilegios necesarios.');
        }
        else $this->error('Usted no está logueado');
	}*/
	...
	function grupos($rfc) //METODO PARA LA BARRA DE PROMOTOR MUESTR GRUPOS
	{
		...
		{
			$sql="select letra from abecedario where cve in (select cveletra from lectura where nocontrol='".$rfc."')";	
		}
		...
		else
		{
			...
			{ 
				$cadena.='<li><a href= "grupo.php?info1='.$datos_rs[$i][0].'">'.$datos_rs[$i][0].'</a></li> ';
			}
			return $cadena;
		}
