...
if(isset($_GET['accion'])) {
	...
	switch($_GET['accion']){
		...
		case 'horario':
			...
			if(!isset($_GET['info'])) {
				$web->smarty->assign('alert', 'danger');
        $web->smarty->assign('msg', 'No se especificó la sala');
        break;
			}
			...
			if(!isset($sala[0])) {
				$web->smarty->assign('alert', 'danger');
        $web->smarty->assign('msg', 'No existe la sala seleccionada');
        break;
			}
		...
		case 'insert':
			...
			if(!verificaUbicacion(1, $web)) { break; }
		    ...
		    if($_POST['datos']['horas'.$i.'_0'] == $_POST['datos']['horas'.$i.'_1'] 
		       && $_POST['datos']['horas'.$i.'_0'] != -1) {
					...
					$web->smarty->assign('alert', 'danger');
      		$web->smarty->assign('msg', 'No duplique los horarios en un mismo día');	
      		$flag = false;
				}
			}
			if(!$flag) { break; }
			...
			if(!isset($_POST['datos']['cvesala']) ||
			   $_POST['datos']['cvesala'] == ""){
			  	$web->smarty->assign('alert', 'danger');
	        $web->smarty->assign('msg', 'No alteres la estructura de la interfaz');
	        break;
			}
		...
		default:
			$web->smarty->assign('alert', 'danger');
      $web->smarty->assign('msg', 'No existe la acción seleccionada');
	}
}
...
$file = fopen("promosala.txt", "w");
fwrite($file, $datos);

$web->smarty->assign('datos', $datos);
$web->smarty->display("promosala.html");

function verificaUbicacion($op, $web, $elementos=null) {
	$cont = 0;
	for ($i = 1; $i <= 6; $i++) {
		for ($j = 0; $j < 2; $j++) {
			...
			switch($op) {
				...
				case 3: //checa que no se duplique periodo, horas y dias con la ubicación
					...
					if($_POST['datos']['horas'.$i.'_'.$j] != -1) {
							...
							if(isset($datos[0])) {
								$web->smarty->assign('alert', 'danger');
		        		$web->smarty->assign('msg', 'La sala u horario ya están ocupados');	
								return false;
							}
				 		}
				break;
